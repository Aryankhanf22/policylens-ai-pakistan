...
**AI-powered economic policy simulation for Pakistan — model fuel, tax, and subsidy shocks, then see the real household and business impact.**

[![React](https://img.shields.io/badge/React-61DAFB)](https://react.dev/)
...
[![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-06B6D4)](https://tailwindcss.com/)
[![Firebase](https://img.shields.io/badge/Firebase-FFCA28)](https://firebase.google.com/)
[![Google Gemini](https://img.shields.io/badge/Google_Gemini-4285F4)](https://ai.google.dev/)

![Overview](Screenshot/home.png)
...
- [Data and Logic Layer](#data-and-logic-layer)
- [AI Features](#ai-features)
- [Firebase and Firestore](#firebase-and-firestore)
- [Project Structure](#project-structure)
- [Getting Started](#getting-started)
- [Notes on Files in the Root](#notes-on-files-in-the-root)
...
|---|---|
| ![Overview](Screenshot/overview.png) |![Overview](Screenshot/business.png) |

| Market Analysis | Habit Log |
|---|---|
|![ Market Analysis](Screenshot/marketanalysis.png)  |![Habit Log ](Screenshot/habit.png) |
...
[src/components/LandingPage.tsx](src/components/LandingPage.tsx) is the public marketing page shown before login. It includes:
...
[src/components/Dashboard.tsx](src/components/Dashboard.tsx) is the main overview screen. It shows:
...
[src/components/ScenarioLab.tsx](src/components/ScenarioLab.tsx) is the core policy simulation tool. Users can:
...
The screen calculates and displays estimated inflation, risk level, household impact in PKR, confidence score, fuel shock percentage, and sector sensitivity indicators for transport, food, agriculture, and retail.

### Business Hub

[src/components/BusinessHub.tsx](src/components/BusinessHub.tsx) focuses on business cost exposure. It lets the user:
...
[src/components/HabitLog.tsx](src/components/HabitLog.tsx) is a personal fuel expense tracker. It supports:
...
[src/components/Analytics.tsx](src/components/Analytics.tsx) presents static analytics views, including fuel price trajectory charts, inflation projection charts, a policy trade-off matrix, and a confidence score panel. This screen is mainly for presentation and comparison rather than data entry.
...
[src/types.ts](src/types.ts) defines the Firestore document shapes used by the app, keeping reads and writes consistent across screens:
...
The AI layer lives in [src/services/gemini.ts](src/services/gemini.ts). It uses the `GEMINI_API_KEY` environment variable and the `@google/genai` client.
...
The Firebase setup is in [src/services/firebase.ts](src/services/firebase.ts). It initializes the Firebase app instance, the Firestore database connection, and the Google authentication provider, and includes a connection test and a shared error handler for Firestore operations.

| Collection | Used By |
|---|---|
| `simulations` | Scenario Lab results saved by signed-in users |
| `businessLogs` | Business Hub fuel-impact analyses |
| `habitLogs` | Habit Log fuel spending entries |
...
This project uses the Firebase config stored in [firebase-applet-config.json](firebase-applet-config.json). Make sure your Firebase project has:
...
This project uses `tsc --noEmit` for the lint script, so it's effectively a type check.

## Notes on Files in the Root

| File | Purpose |
|---|---|
| `firebase-blueprint.json` | Firebase project metadata used by the workspace |
| `firebase-applet-config.json` | Runtime Firebase app config |
| `firestore.rules` | Firestore access rules |
| `metadata.json` | Project metadata for the workspace |
| `vite.config.ts` | Configures Vite, Tailwind, the alias, and environment injection |
...
- Use Gemini to turn raw numbers into readable guidance
